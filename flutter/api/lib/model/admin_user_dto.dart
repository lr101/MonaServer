...
class AdminUserDto {
  /// Returns a new [AdminUserDto] instance.
  AdminUserDto({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is AdminUserDto &&
  ...
  /// Returns a new [AdminUserDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AdminUserDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AdminUserDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AdminUserDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AdminUserDto> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AdminUserDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AdminUserDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AdminUserDto> mapFromJson(dynamic json) {
    final map = <String, AdminUserDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AdminUserDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AdminUserDto-objects as value to a dart map
  static Map<String, List<AdminUserDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AdminUserDto>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = AdminUserDto.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
