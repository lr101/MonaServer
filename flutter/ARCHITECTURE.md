...
| Sign in and signup | [`auth.dart`](lib/features/auth/presentation/auth.dart) provides a custom sign-in screen with email-link and username/password options, plus a signup form with username, password, matching email confirmation, and required Terms and Privacy acknowledgements. Signup and password sign-in retain the existing v2 API contract; email-link delivery and callback admission retain the public email-login flow. Native callback transport is not implemented. | `email_login_domain_test.dart`, `email_login_screen_test.dart`, `email_login_data_test.dart` |
