...
| Sign in and signup | [`auth.dart`](lib/features/auth/presentation/auth.dart) provides an animated sign-in card with email-link and username/password options, plus signup with username, password, matching email confirmation, and required Terms and Privacy acknowledgements. Usernames cannot contain `@`, so the email-link field infers identifier type. The app shows a specific unavailable message when the server's email-login flag is off. Signup and password sign-in retain the v2 API contract; link delivery and callback admission retain the public email-login flow. Android App Links capture email callbacks before bootstrap and handle incoming links while the app is running. | `email_login_domain_test.dart`, `email_login_screen_test.dart`, `email_login_data_test.dart` |
| Android group invites | Private-group invite links open the group route with the invite code held in memory through sign-in. The group screen submits the code only after the user taps Join; group invite controls copy both the existing code and a shareable link. Android link verification also requires the production domain to serve `.well-known/assetlinks.json` containing the release signing certificate fingerprint. | — |
