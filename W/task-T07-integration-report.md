...
takeover MFA`). The implementation
...
attempted because it would remove the T07 lane and exceed this service-only
fix scope.
...
own authenticated request, `jobs.execute_all` for execution or
`jobs.control_all` alongside `jobs.control` for retry/resume, and a fresh MFA
proof bound to the job action. The service never copies the creator's proof
...
if renewal fails. Lease loss then uses a separate exact-token/fence terminal
commit that tolerates expiry, records `unknown_delivery` plus its audit intent
atomically, and clears the claim so a reclaim cannot repeat the side effect. If
...
```text
$ git rev-parse HEAD
286aa8ea476b30342e608005c4e5324bade39af8
...
$ (cd go-server && mise exec -- go test -race -count=1 -p 1 ./internal/service ./internal/handler ./cmd/server -run '^(Test(Bulk|Admin|T07|Report|EndpointReport|RealAdminRouterUsesBrowserSessionBoundary|WebAdminAPI|V3))')
PASS — focused service, handler, and server route race checks
...
This follow-up changes only the bulk service and its focused tests; the
earlier disposable PostGIS serial evidence remains applicable because no DB,
SQL, migration, API, generated, handler, or router file changed in this fix.
