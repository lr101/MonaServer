...
function renderShell(value) {
  const pages = ['overview', 'users', 'reports', 'audit'];
...
function bindPage() { root.querySelector('#user-search')?.addEventListener('submit', searchUsers); root.querySelector('#report-search')?.addEventListener('submit', searchReports); root.querySelector('#audit-search')?.addEventListener('submit', searchAudit); root.querySelector('#report-update')?.addEventListener('submit', updateReport); root.querySelector('#note-form')?.addEventListener('submit', addNote); root.querySelector('[data-action="back"]')?.addEventListener('click', () => state.update({ detail: null, notes: null })); root.querySelectorAll('[data-user-id]').forEach((button) => button.addEventListener('click', () => loadUser(button.dataset.userId))); root.querySelectorAll('[data-report-id]').forEach((button) => button.addEventListener('click', () => loadReport(button.dataset.reportId))); root.querySelector('[data-next-page]')?.addEventListener('click', (event) => loadPage(event.currentTarget.dataset.nextPage, { ...state.value.filters, cursor: event.currentTarget.dataset.cursor })); }
async function selectPage(page) { state.update({ page, detail: null, notes: null, items: [], nextCursor: null, error: null, filters: {} }); if (page !== 'overview') await loadPage(page); }
async function loadPage(page, options = {}) { state.update({ page, busy: true, error: null }); try { const result = page === 'users' ? await api.listUsers(options) : page === 'reports' ? await api.listReports(options) : await api.listAudit(options); state.update({ busy: false, loaded: true, items: result?.items ?? [], nextCursor: result?.nextCursor ?? null }); } catch (error) { handle(error); } }
