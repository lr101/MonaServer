...
Implementation complete; task review is pending. The slice is based on the
reviewed contract amendment at `922a12d` and keeps generated/API, Go, router,
and T10 shared files unchanged.
...
- `flutter/test/features/admin_reports/admin_reports_controller_test.dart`
  - Paging/filter races, detail updates/notes, blank-assignee clearing,
    revision conflict, selected/all-matching previews, stale bulk work, text
...
- `flutter/test/features/admin_reports/admin_reports_screen_test.dart`
  - Deleted target, detail controls, permission denial, loading/empty/error,
    narrow-layout status filter, and text-search bulk-action accessibility.
...
all-matching actions remain supported.
...
- `mise exec -- dart format lib/features/admin_reports lib/widgets/report_issue/report_issue_submission.dart test/features/admin_reports` — passed.
- `mise exec -- flutter test --no-pub test/features/admin_reports` — passed,
  19 tests.
- `mise exec -- flutter analyze lib/features/admin_reports lib/widgets/report_issue/report_issue_submission.dart test/features/admin_reports` — passed with no issues.
...
PostGIS, RustFS, SMTP, push, or browser stack was started. Live service-backed
report persistence and SMTP-unavailable success remain integration checks for
T10/T13; no production credentials or provider payloads were accessed.
...
- The consumer report page still uses its existing legacy service call until
  the scheduled composition transfer wires `ReportIssueSubmission.toReportDto()`
  into the transport. The additive mapping and compatibility tests are owned
  here; the generated client and global service remain untouched.
